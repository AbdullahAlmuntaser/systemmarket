...
class _EmployeesPageState extends State<EmployeesPage> {
  ...
  Widget _buildEmployeeCard(Employee emp, HRProvider provider, ColorScheme colorScheme) {
    return Card(
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(emp.name[0].toUpperCase(), style: TextStyle(color: colorScheme.onPrimaryContainer, fontWeight: FontWeight.bold, fontSize: 20)),
                ),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(emp.jobTitle ?? 'بدون مسمى وظيفي', style: TextStyle(color: colorScheme.outline, fontSize: 14)),
                      const SizedBox(height: 4),
                      Text('كود: ${emp.employeeCode}', style: TextStyle(fontSize: 12, color: colorScheme.outline)),
                    ],
                  ),
                ),
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('الراتب الأساسي', style: TextStyle(fontSize: 10, color: Colors.grey)),
                    Text('${emp.basicSalary.toStringAsFixed(2)} ر.س', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: colorScheme.primary)),
                  ],
                ),
                Row(
                  children: [
  ...
  Widget _buildStatusBadge(bool isActive) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: (isActive ? Colors.green : Colors.red).withAlpha(100)),
      ),
      child: Text(
        isActive ? 'نشط' : 'متوقف',
  ...
  void _confirmDelete(Employee emp, HRProvider provider) {
    showDialog(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () { provider.deleteEmployee(emp.id); Navigator.pop(ctx); }, style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white), child: const Text('حذف')),
      ],
    ));
  }
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              TextField(controller: salaryController, decoration: const InputDecoration(labelText: 'الراتب الأساسي'), keyboardType: TextInputType.number),
            ],
          ),
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (employee == null) {
                provider.addEmployee(EmployeesCompanion.insert(id: Value(const Uuid().v4()), name: nameController.text, employeeCode: codeController.text, jobTitle: Value(jobTitleController.text), basicSalary: Value(double.tryParse(salaryController.text) ?? 0.0), isActive: const Value(true)));
              } else {
                provider.updateEmployee(employee.copyWith(name: nameController.text, employeeCode: codeController.text, jobTitle: Value(jobTitleController.text), basicSalary: double.tryParse(salaryController.text) ?? 0.0));
              }
